...
PatientSpanAE <- function(baseEI, baseTr, baseDates, baseBio, choixUSU, suivi=TRUE, CODvar="PT_COD", listcol, bio=FALSE){
  ...
  df_pat <- df_pat %>% select(-c(RANDO_DAT,AE_INIT_NUM,AE_INIT))
  ...
  names(df_pat) <- c("AE_CJP","AE_ONGO","SAE","AE_GRADE","AE_MOL1","AE_MOL2","AE_OUT","AE_MOL2_CVE","AE_CVE","COD","SOC_COD","aestartdate","aeenddate" )
